class customProductVariant extends HTMLElement {
  constructor() {
    super();
  ...
  priceCalculate(){
    ...
    const markupDataElement = document.getElementById('markup');
    const markupData = JSON.parse(markupDataElement.textContent);
      ...
      $(this).find('input').each(function () {
        ...
        if (!isNaN(inputValue) && inputValue > 0) {
          ...
          if (!isNaN(variantPrice)) {
            ...
            count_total_price += totalQtyPrice;
          }
        }
      });
      var total_qty = parseInt($(this).find('.chose-size__optons--totalprice .chose-size__qtyvalue').val());
      var new_price_with_markup = 0;

      var markup = markupData.find(function(item) {
        return total_qty <= item.pcs;
      });

      debugger
      new_price_with_markup = count_total_price*markup.markup
      allVariantPrice += new_price_with_markup;
    });
    ...
    $('.active_method[print-method-data-name]').each(function() {
      ...
      console.log(TotalColorQty+" TotalColorQty");
      if (targetObject) {
    ...
    console.log(allVariantPrice.toFixed(2));
  ...
  onSubmitHandler(evt) {
    ...
    prined_part.each((index, itemPart) => {
      ...
      console.log(printActiveproductID);
    ...
    this.colorElements.forEach(colorSection => {
      ...
      sizeOptions.forEach(sizeOption => {
        const inputField = sizeOption.querySelector('.chose-size__qtyvalue');
        const colorName = $(sizeOption).parents('.chose-size').data('color-value');
...
customElements.define('custom-product-variant', customProductVariant);